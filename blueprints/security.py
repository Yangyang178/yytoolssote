...
class _LazyAppImports:
    def __getattr__(self, name):
        from app import get_db, log_message, api_response, get_user_storage_usage
        _mapping = {
            'get_db': get_db,
...
def check_password_strength():
    ...
    if not password:
        return _app.api_response(success=False, message='密码不能为空')
...
def verify_2fa():
    ...
    if not code:
        return _app.api_response(success=False, message='请输入验证码')
    ...
    try:
        two_fa = conn.execute("SELECT * FROM user_2fa WHERE user_id = ? AND verified = 0",
                             (session['user_id'],)).fetchone()
        if not two_fa:
            return _app.api_response(success=False, message='未找到待验证的2FA设置')

        totp = pyotp.TOTP(two_fa['secret'])
        if not totp.verify(code, valid_window=1):
            return _app.api_response(success=False, message='验证码无效')
        ...
        return _app.api_response(success=True, message='两步验证已启用')
    finally:
        conn.close()
...
def disable_2fa():
    ...
    if not password:
        return _app.api_response(success=False, message='请输入密码确认')
    ...
    try:
        ...
        if not check_password_hash(user['password'], password):
            return _app.api_response(success=False, message='密码不正确')
        ...
        return _app.api_response(success=True, message='两步验证已禁用')
    finally:
        conn.close()
...
def get_security_events():
    ...
    try:
        ...
        return _app.api_response(success=True, data={'events': events})
    finally:
        conn.close()
...
def get_login_devices():
    ...
    try:
        ...
        return _app.api_response(success=True, data={'devices': device_list})
    finally:
        conn.close()
...
def trust_device(device_id):
    ...
    try:
        ...
        if not device:
            return _app.api_response(success=False, message='设备不存在', code=404)

        conn.execute("UPDATE login_devices SET is_trusted = 1 WHERE id = ?", (device_id,))
        conn.commit()

        return _app.api_response(success=True, message='设备已标记为信任')
    finally:
        conn.close()
...
def remove_device(device_id):
    ...
    try:
        ...
        if result.rowcount == 0:
            return _app.api_response(success=False, message='设备不存在或已被删除')
        ...
        return _app.api_response(success=True, message='设备已移除')
    finally:
        conn.close()
